    ...
    if 'access_token' in session:
        ...
        while not message:
            try:
                ...
                #message = client.messages.first() # message = client.messages.where(**{'in': 'all', 'limit': 5}).first()
                for message in client.messages.where(**{'limit': 5})
        ...
        #text += "</html>"
        #for thread in client.threads.where(**{'in': 'all', 'unread':True, 'limit': 5}):
        #    print thread.date
